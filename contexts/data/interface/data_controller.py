...
def _parse_filters(request) -> list[FilterCriterion]:
    ...
    for raw in request.args.getlist("filter"):
        parts = raw.split(":", 2)
        if len(parts) == 3:
            filters.append(FilterCriterion(field=parts[0], operator=parts[1], value=parts[2]))
    return filters
...
class DataController(BaseController):
    ...
    def __init__(self, data_svc: DataApplicationService, access_policy: ProjectAccessPolicy):
        super().__init__()
        self.svc = data_svc
    ...
    async def query(self, request, template_id: str):
        ...
        return self.json(
            await self.svc.query(template_id, batch_id=batch_id, pagination=p, filters=_parse_filters(request))
        )
