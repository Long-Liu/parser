...
def _entity(row: AlertModel) -> Alert:
    return Alert(
        ...
        threshold_value=Decimal(row.threshold_value),
...
class TortoiseAlertRepository(AlertRepository):
    ...
    async def rules(self) -> list[AlertRule]:
        if not await AlertRuleModel.exists():
            for code, name, metric, operator, threshold, level in DEFAULT_RULES:
                await AlertRuleModel.get_or_create(
                    code=code,
                    defaults={
                        "name": name,
                        "metric": metric,
    ...
    async def save(self, alert: Alert) -> None:
        ...
        if alert.id is None:
            row = await AlertModel.create(**values)  # type: ignore[arg-type]
            alert.id = row.id
        else:
            await AlertModel.filter(id=alert.id).update(**values)

    async def record_event(self, alert: Alert, event_type: str, actor_id: int | None = None, note: str = "") -> None:
    ...
    async def find(
        ...
        total = await query.count()
        rows = await query.order_by("-last_triggered_at", "-id").offset(pagination.offset).limit(pagination.size)
    ...
    async def delete_project(self, project_id: int) -> None:
        alert_ids = list(await AlertModel.filter(project_id=project_id).values_list("id", flat=True))
        if alert_ids:
    ...
    async def missed_outbox(self, project_ids: list[int], since: str | None) -> list[dict]:
        ...
        return [
            {"event": row.event_type, "event_id": row.id, "data": row.payload, "project_id": row.project_id}
            for row in rows
        ]
...
class TortoiseAlertMetricProvider(AlertMetricProvider):
    async def snapshot(self, project_id: int, ym: str | None = None) -> tuple[str | None, dict[str, Decimal]]:
        ...
        if batch:
            ...
            indicators = {row.indicator_name: row.cumulative_value for row in settlement if row.indicator_name}
