...
CATALOG: dict[str, dict[str, EndpointDoc]] = {
    ...
    "AnalyticsController": {
        ...
        "budget_lease_writeoffs": doc("预算租借核销报表", "按项目和月份汇总预算租借、已核销及剩余未核销金额。", query=(q("project_ids", "项目 ID，逗号分隔"), q("ym", "月份，格式 YYYY-MM")), paginated=True),
        ...
        "export_budget_lease_writeoffs": doc("导出预算租借核销", "按项目和月份导出预算租借核销汇总 Excel。", query=(q("project_ids", "项目 ID，逗号分隔"), q("ym", "月份，格式 YYYY-MM")), response=XLSX, response_description="Excel 文件"),
