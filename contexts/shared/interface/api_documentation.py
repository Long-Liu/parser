...
XLSX = {"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": bytes}
...
CATALOG: dict[str, dict[str, EndpointDoc]] = {
    "AuthController": {
        ...
        "register": doc(
            "创建账号",
            "创建系统账号；关闭开放注册时需要用户管理权限。",
            body=BODIES["register"],
            status=201,
            secured=False,
        ),
    ...
    "ProjectsController": {
        "list_projects": doc(
            "项目列表",
            "按名称或编码关键字、状态筛选可访问项目。",
            query=(q("keyword", "项目名称或编码关键字"), q("status", "项目状态")),
            paginated=True,
        ),
        "create_project": doc(
            "创建项目",
    ...
    "UploadsController": {
        "upload": doc(
            ...
            body_description="Excel 文件、项目 ID 和月份",
            response_description="解析批次结果",
        ),
        "preview": doc(
            "预览 Excel 解析结果",
            "上传 .xlsx 文件并生成待确认的解析预览。",
            body={
                "multipart/form-data": {
                    "file": bytes,
                    "project_id": int,
    ...
    "DataController": {
        "query": doc(
            ...
            "按模板、批次和动态过滤条件查询解析数据。",
            query=(
                q("batch_id", "上传批次 ID", int),
                q("filter", "过滤条件：字段:操作符:值，可重复"),
            ),
            paginated=True,
        ),
    ...
    "TemplatesController": {
        ...
        "download_template": doc(
            "下载 Excel 模板",
            "下载指定模板生成的 Excel 空白文件。",
            response=XLSX,
            response_description="Excel 文件",
        ),
    },
    "AlertController": {
        "list_alerts": doc(
    ...
    "AnalyticsController": {
        ...
        "month_comparison": doc(
            "项目月份对比",
            "对比指定项目的多个月份数据。",
            body=BODIES["months"],
        ),
        "delete_monthly_data": doc(
            "删除月度数据",
            "删除指定项目月份的解析数据。",
            response=OK,
        ),
        "compare_projects": doc(
            "多项目对比",
            "对比多个项目在指定月份的指标。",
            body=BODIES["compare"],
        ),
        ...
        "budget_lease_writeoffs": doc(
            "预算租借核销报表",
            "按项目和月份汇总预算租借、已核销及剩余未核销金额。",
            query=(
                q("project_ids", "项目 ID，逗号分隔"),
                q("ym", "月份，格式 YYYY-MM"),
            ),
        ...
        "global_search": doc(
            "全局搜索",
            "按关键字搜索项目和业务数据。",
            query=(q("keyword", "搜索关键字", required=True),),
        ),
        ...
        "export_budget_lease_writeoffs": doc(
            "导出预算租借核销",
            "按项目和月份导出预算租借核销汇总 Excel。",
            query=(
                q("project_ids", "项目 ID，逗号分隔"),
                q("ym", "月份，格式 YYYY-MM"),
            ),
        ...
        "export_month_comparison": doc(
            "导出月份对比",
            "导出指定项目的月份对比 Excel。",
            query=(q("months", "月份，逗号分隔", required=True),),
            response=XLSX,
            response_description="Excel 文件",
        ),
        "export_compare": doc(
            ...
            "导出多项目对比 Excel。",
            query=(
                q("project_ids", "项目 ID，逗号分隔", required=True),
                q("ym", "月份，格式 YYYY-MM"),
            ),
            response=XLSX,
            response_description="Excel 文件",
        ),
        "compare_ai_analysis": doc(
            "多项目 AI 分析",
            "生成多项目对比的 AI 分析结果。",
            body=BODIES["compare"],
        ),
...
def apply_controller_docs(controller) -> None:
    ...
    for method_name, endpoint in endpoints.items():
        ...
        for name, path_parameter in signature(handler).parameters.items():
            if name in {"request", "ws"}:
                continue
            annotation = type_hints.get(name, path_parameter.annotation)
            schema = annotation if annotation in {str, int, float, bool} else str
            handler = openapi.parameter(name, schema, "path", description=f"{name} 路径参数", required=True)(handler)

        parameters = endpoint.query + (PAGE if endpoint.paginated else ())
        for query_parameter in parameters:
        ...
        if endpoint.body is not None:
            ...
            if not (isinstance(body_content, dict) and any("/" in key for key in body_content)):
