...
## 9. Figma UI 更新跟进（2026-07-29）

本次重新检查发布站点后，确认数据分析菜单新增独立页面：
`/budget-lease-writeoff`「预算租借核销表」。页面展示项目级汇总和合计行，
字段为预算租借合计、累计租借三分类、已核销、未核销、剩余未核销三分类，
并提供“导出报表”操作。

后端已同步补齐：

- `GET /api/reports/budget-lease-writeoffs`：支持 `project_ids`、`ym` 和分页；
- `GET /api/reports/budget-lease-writeoffs/export`：导出与页面字段一致的 Excel；
- 数据来自 `data_budget_lease`（表10.3），每个项目只取最新成功上传批次，
  避免重复上传造成重复累计；
- 接口沿用项目权限范围，普通用户只能看到被授权项目；
- 返回 `summary` 合计和 `projects` 明细，三分类将源表定标/有源/无源口径
  映射为页面的机械设备租赁/周转材料租借/其他租借费用。

## 10. 当前发布 UI 逐页复核与补齐（2026-07-29）

本次通过发布页面逐页操作复核了登录、项目总览、新增项目、项目详情四个
Tab、数据管理、数据表格三个 Tab、多项目对比、项目毛利、预算租借核销、
成本科目（表格/图表）、人员管理新增表单和数据大屏。

| 页面 | 复核结果 | 本次处理 |
|---|---|---|
| 登录 | 登录/当前用户/退出均有接口；“忘记密码”仍无邮件基础设施 | 保留管理员重置和本人改密，不伪造邮件找回 |
| 项目总览 | 列表、KPI、搜索、状态筛选、经营指标匹配；UI 新增表单无项目编号 | 后端在 `code` 为空时生成 `PRJ-XXXXXXXX`，兼容 `contract/startDate/leader/notes` 别名并按负责人姓名解析用户 |
| 项目详情·单项目 | 基础/动态/预计完工已具备；指标毛利和租借核销此前为空 | 指标毛利取动态指标成本，租借结算/成本/毛利/核销率取表10.3 租借总额/已核销/未核销 |
| 项目详情·月度对比 | 核心指标和环比具备；缺 UI 的每月成本构成 | 每个月新增 `cost_categories`，返回实际已发生和预计完工 |
| 项目详情·数据详情 | UI 展示 32 列，模型/模板只保存前 26 列 | 模型、YAML 和迁移 0008 补显示级别、链接分表、调整税率/税金、预计完工成本四列及末列备注；重复“备注”按出现次数正确映射 |
| 项目详情·AI | 单项目 AI 已有，但未携带预计完工和核销信息 | AI 输入和确定性兜底增加预计完工毛利率与租借核销率 |
| 数据管理/上传 | preview/confirm/cancel、错误行、删除、模板下载均匹配 | 无新增缺口 |
| 数据表格 | 月度经营、里程碑、成本三口径均可读取；通用行更新接口可保存 | 新增的动态指标字段自动进入现有 GET/PUT 数据接口白名单 |
| 多项目对比 | 9 指标、五维评分、AI 五章节、导出均匹配 | 无新增缺口；营收与结算仍因源表无独立营收列而同源 |
| 项目毛利 | 四口径和导出结构匹配 | 投标口径源文件没有报价表，继续返回兼容空值，不制造业务数据 |
| 预算租借核销 | 新页面 | 见第 9 节 |
| 成本科目 | 表格、层级、导出匹配；图表可直接复用同一接口数据 | 预计完工优先使用新增 `forecast_with_tax`，旧数据回退预计完工量指标 |
| 人员管理 | UI 只提交姓名/邮箱/部门/管理员，旧接口要求账号密码 | 邮箱可作为账号，自动生成一次性初始密码并仅在创建响应返回；自动分配 admin/viewer 系统角色，列表补负责人/查看数量 |
| 数据大屏 | KPI、趋势、分布、构成、状态匹配；雷达字段仍是旧 5 维 | 对齐为进度/成本/质量/安全/效率/毛利六维；质量安全暂无独立源，以无预警率作为明确兜底 |
| 全局搜索/通知/同步 | 搜索、未读、已读、删除、清空、同步状态和 WS 均有接口 | 无新增缺口 |

仍需产品或新数据源才能真实完成的只有三项：邮箱“忘记密码”、独立营收口径、
投标报价口径；当前实现不会用模拟值冒充真实数据。
